...
# Semester-Aware Course Catalogues
SEMESTER_COURSES = {
    ...
    "3rd Semester": ["COAL(Lab)", "COAL", "OOP (Lab)", "OOP", "Calculus", "LA", "Mngmnt (2hr)"],
    "4th Semester": ["CN (Lab)", "C.Networks", "SE (Lab)", "SE", "TBW", "S/w Req Engg.", "SRE"],
    "5th Semester": ["DB (Lab)", "DB", "HCI (Lab)", "HCI", "S/w Design & Archi.", "SDA (Lab)", "SDA", "Web P. (Lab)", "Web P."],
    "6th Semester": ["OS (Lab)", "OS", "S/w Quality", "SQE", "Info. Security", "IS", "S/w Project Mngmnt", "SPM"],
    "7th Semester": ["MAD", "ML", "PDC", "Data Mining", "Freelancing", "Info. Assur.", "CV", "Deep L", "Q.A.", "NLP", "GT", "TOC"],
    "8th Semester": ["PPIT", "CC", "DIP", "FYP-1", "AI", "WE", "E-Com", "DDBS", "MAD", "ML"]
}

# Fallback global courses for safety, ordered by length
...
def parse_cell(text, semester):
    ...
    # 3. Context-Aware Deterministic Matching
    course = None
    instructor = None
    unresolved = False
    ...
    # Try semester-specific list first
    candidate_courses = SEMESTER_COURSES.get(semester, [])
    # Sort by length descending to match longer specific names first (e.g. "DLD (Lab)" before "DLD")
    candidate_courses = sorted(candidate_courses, key=len, reverse=True)
    
    matched_course = None
    
    for kc in candidate_courses:
        # Use explicit word boundaries \b for strict matching.
        # Handle special characters like ( ) in names by using custom boundary check
        escaped_kc = re.escape(kc)
        # Regex: boundary (or start of string), the course, boundary (or end of string)
    ...
    # If not found in semester list, try global list as fallback (some sections might have mixed semester courses)
    ...
    if matched_course:
        ...
        # Remove course from text to get instructor
